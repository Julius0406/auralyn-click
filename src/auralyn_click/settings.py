...
LEGACY_SETTINGS_PATH = Path.home() / ".config" / "pulseclick" / "settings.json"
...
def migrate_legacy_settings() -> None:
    """Import the old PulseClick JSON once, without deleting it."""
    if SETTINGS_PATH.exists() or not LEGACY_SETTINGS_PATH.exists():
        return
    try:
        CONFIG_DIR.mkdir(parents=True, exist_ok=True)
        with LEGACY_SETTINGS_PATH.open("r", encoding="utf-8") as src:
            old = json.load(src)
