...
def monitor_entries() -> list[tuple[str, int]]:
    """Return user-facing monitor labels and stable 0-based Qt screen indexes."""
    screens = list(QGuiApplication.screens())
    primary = QGuiApplication.primaryScreen()
    result: list[tuple[str, int]] = []
    for index, screen in enumerate(screens):
        geo = screen.geometry()
        name = (screen.name() or f"Display {index + 1}").strip()
        primary_suffix = " • Primary" if screen is primary else ""
        result.append(
            (f"Monitor {index + 1} — {name} — {geo.width()}×{geo.height()}{primary_suffix}", index)
        )
    if not result:
        result.append(("Monitor 1", 0))
    return result


def selected_screen(settings: dict):
    screens = list(QGuiApplication.screens())
    if not screens:
        return QGuiApplication.primaryScreen()
    try:
        index = int(settings.get("monitor_index", 0))
    except Exception:
        index = 0
...
def center_widget_on_selected_screen(widget: QWidget, settings: dict) -> None:
    screen = selected_screen(settings)
    if screen is None:
        return
    try:
        widget.winId()  # ensure a native window handle exists
...
class SetupWizard(QWizard):
    ...
    def _build_startup(self) -> None:
        ...
        self.setup_monitor = QComboBox()
        for label, index in monitor_entries():
            self.setup_monitor.addItem(label, index)
        monitor_idx = self.setup_monitor.findData(int(self.settings.get("monitor_index", 0)))
        self.setup_monitor.setCurrentIndex(max(0, monitor_idx))
        self.setup_monitor.currentIndexChanged.connect(self._move_to_setup_monitor)
        ...
        form.addRow("Monitor", self.setup_monitor)
    ...
    def _move_to_setup_monitor(self, *_args) -> None:
        try:
            temp = dict(self.settings)
            temp["monitor_index"] = int(self.setup_monitor.currentData() or 0)
            center_widget_on_selected_screen(self, temp)
        except Exception:
            pass
    ...
    def accept(self) -> None:
        self.settings.update({
            ...
            "monitor_index": int(self.setup_monitor.currentData() or 0),
...
class SplashScreen(AnimatedBackground):
    ...
    def __init__(self, settings: dict):
        ...
        if settings["startup_screen"] == "fullscreen":
            ...
            self.resize(900, 520)
            center_widget_on_selected_screen(self, settings)
            self.show()
...
class SettingsDrawer(QFrame):
    ...
    def __init__(self, owner: "AuralynWindow"):
        ...
        for label, index in monitor_entries():
            self.monitor.addItem(label, index)
        monitor_idx = self.monitor.findData(int(owner.settings.get("monitor_index", 0)))
        self.monitor.setCurrentIndex(max(0, monitor_idx))
        ...
        sg.addWidget(QLabel(owner.t("monitor")), 2, 0); sg.addWidget(self.monitor, 2, 1)
        startup.content.addLayout(sg)
        monitor_note = QLabel(owner.t("monitor_note"))
        monitor_note.setWordWrap(True); monitor_note.setObjectName("muted")
        startup.content.addWidget(monitor_note)
        ...
        for widget in [self.theme, self.accent, self.background, self.fps, self.startup_mode, self.monitor]:
...
class AuralynWindow(QMainWindow):
    ...
    def _on_appearance_changed(self) -> None:
        values = self.drawer.values()
        old_monitor = int(self.settings.get("monitor_index", 0))
        new_monitor = int(values.get("monitor_index", old_monitor))
        self.settings.update(values)
        if new_monitor != old_monitor:
            QTimer.singleShot(0, self._position_on_selected_monitor)
        self.apply_appearance()
        self.schedule_save()
    ...
    def _position_on_selected_monitor(self) -> None:
        try:
            center_widget_on_selected_screen(self, self.settings)
        except Exception:
            pass

    def show_with_animation(self) -> None:
        self._position_on_selected_monitor()
        self.show()
        self.raise_()
        self.activateWindow()
