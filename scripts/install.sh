...
if command -v apt >/dev/null 2>&1; then
  echo "==> Installing/checking Ubuntu runtime packages..."
  sudo apt update
  sudo apt install -y python3 python3-venv python3-pip libxcb-cursor0
fi

# Stop old instances if they exist.
if [ -x "$BIN_DIR/auralyn-click-control" ]; then
  "$BIN_DIR/auralyn-click-control" stop 2>/dev/null || true
fi
...
# Copy source, not just a binary: this keeps the local install auditable/editable.
...
Comment=Modern animated Linux auto clicker by Julius
...
if [ -z "${DESKTOP_DIR:-}" ]; then
  [ -d "$HOME/Schreibtisch" ] && DESKTOP_DIR="$HOME/Schreibtisch" || DESKTOP_DIR="$HOME/Desktop"
fi
mkdir -p "$DESKTOP_DIR"
cp "$APP_DIR/auralyn-click.desktop" "$DESKTOP_DIR/Auralyn-Click.desktop"
chmod +x "$DESKTOP_DIR/Auralyn-Click.desktop"
if command -v gio >/dev/null 2>&1; then
  gio set "$DESKTOP_DIR/Auralyn-Click.desktop" metadata::trusted true 2>/dev/null || true
fi

# GNOME global shortcuts. F8 is a dedicated stop-only emergency shortcut.
if command -v gsettings >/dev/null 2>&1 && gsettings list-schemas | grep -qx 'org.gnome.settings-daemon.plugins.media-keys'; then
  ...
  NEW_LIST="$(python3 - "$CURRENT" "$TOGGLE_PATH" "$CAPTURE_PATH" "$STOP_PATH" <<'PY'
import ast, sys
raw=sys.argv[1].strip(); new_paths=sys.argv[2:]
if raw.startswith('@as '): raw=raw[4:].strip()
try: arr=ast.literal_eval(raw)
except Exception: arr=[]
if not isinstance(arr,list): arr=[]
# Remove the old PulseClick bindings so F6/F7/F8 are not registered twice.
old_paths={
    '/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/pulseclick-toggle/',
    '/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/pulseclick-capture/',
    '/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/pulseclick-stop/',
}
arr=[p for p in arr if p not in old_paths]
for p in new_paths:
...
printf '   Run: auralyn-click\n'
printf '   Settings: ~/.config/auralyn-click/settings.json\n'
printf '   Old PulseClick settings are imported automatically once.\n\n'
