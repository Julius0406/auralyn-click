...
begin
  ...
  AddLabel(StartupPage, 'Monitor for loading screen and app window', 180);
  MonitorCombo := NewCombo(StartupPage, 200);
  I := GetSystemMetrics(SM_CMONITORS);
  if I < 1 then
    I := 1;
  while MonitorCombo.Items.Count < I do
    MonitorCombo.Items.Add('Monitor ' + IntToStr(MonitorCombo.Items.Count + 1));
...
begin
  ...
  Json := '{' + #13#10 +
    ...
    '  "monitor_index": ' + IntToStr(MonitorCombo.ItemIndex) + ',' + #13#10 +
