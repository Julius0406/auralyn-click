...
```text
AuralynClick-2.4.4-Windows-x64-Setup.exe
```
...
```text
AuralynClick-2.4.4-linux-amd64.deb
```
...
- multi-monitor detection and monitor selection for the loading screen and main window
...
Auralyn Click detects the monitors currently connected to the computer.

The setup wizard and the Display & Design panel show them as Monitor 1, Monitor 2, Monitor 3, and so on, together with the Qt display name and resolution when available. The selected monitor controls both the startup/loading screen and the main application window.

If a saved monitor no longer exists, Auralyn Click safely falls back to an available monitor.
...
```text
AuralynClick-2.4.4-Windows-x64-Setup.exe
AuralynClick-2.4.4-Windows-x64-Portable.zip
AuralynClick-2.4.4-linux-amd64.deb
AuralynClick-2.4.4-linux-x86_64-portable.tar.gz
AuralynClick-2.4.4-GitHub-Source.zip
```
